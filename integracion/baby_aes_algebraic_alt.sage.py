...
# This file was *autogenerated* from the file integracion/baby_aes_algebraic_alt.sage
from sage.all_cmdline import *   # import sage library

_sage_const_0 = Integer(0); _sage_const_8 = Integer(8); _sage_const_16 = Integer(16); _sage_const_1 = Integer(1); _sage_const_2 = Integer(2); _sage_const_3 = Integer(3); _sage_const_4 = Integer(4); _sage_const_0x063b = Integer(0x063b); _sage_const_0x21c2 = Integer(0x21c2)#!/usr/bin/env sage
...
if PROJECT_ROOT not in sys.path:
    sys.path.insert(_sage_const_0 , PROJECT_ROOT)
...
def build_fixed_vars_from_bits(component_plain_id, component_cipher_id, plaintext_bits_le, ciphertext_bits_le):
    ...
    fixed = [{
        ...
        'bit_positions': list(range(_sage_const_0 ,_sage_const_8 )),
        'bit_values': plaintext_bits_le[_sage_const_0 :_sage_const_8 ]
    }, {
        'component_id': component_plain_id,
        'constraint_type': 'equal',
        'bit_positions': list(range(_sage_const_8 ,_sage_const_16 )),
        'bit_values': plaintext_bits_le[_sage_const_8 :_sage_const_16 ]
    }, {
        'component_id': component_cipher_id,
        'constraint_type': 'equal',
        'bit_positions': list(range(_sage_const_0 ,_sage_const_8 )),
        'bit_values': ciphertext_bits_le[_sage_const_0 :_sage_const_8 ]
    }, {
        'component_id': component_cipher_id,
        'constraint_type': 'equal',
        'bit_positions': list(range(_sage_const_8 ,_sage_const_16 )),
        'bit_values': ciphertext_bits_le[_sage_const_8 :_sage_const_16 ]
    }]
    return fixed
...
def build_variant_B_bits_from_le(bits_le):
    ...
    low = bits_le[_sage_const_0 :_sage_const_8 ][::-_sage_const_1 ]   # low byte LSB->MSB revertido -> MSB->LSB
    high = bits_le[_sage_const_8 :_sage_const_16 ][::-_sage_const_1 ]
    return low + high
...
def try_sat_variant(babyAES, plaintext, key_expected, component_plain_id='plaintext', component_cipher_id=None, solver_name="CRYPTOMINISAT_EXT", variant='A'):
    ...
    if component_cipher_id is None:
        ...
        component_cipher_id = babyAES.get_all_components()[-_sage_const_1 ].id

    plaintext_bits_le = to_binary(plaintext, _sage_const_16 )   # LSB-first
    ciphertext = babyAES.evaluate([key_expected, plaintext])
    ciphertext_bits_le = to_binary(ciphertext, _sage_const_16 )
...
def brute_force_find_keys(babyAES, plaintext, ciphertext):
    ...
    matches = []
    # iterar 0..65535
    for k in range(_sage_const_0 , _sage_const_1 <<_sage_const_16 ):
...
def main():
    ...
    expected_key = _sage_const_0x21c2 
    ...
    if statusA == 'SATISFIABLE' and recA is not None:
        ...
        if recA == expected_key or ct_test == ciphertext:
            ...
            print(json.dumps(result, indent=_sage_const_2 ))
            return
    else:
    ...
    print("\nJSON resumen:")
    print(json.dumps(summary, indent=_sage_const_2 ))
